...
function TutorInfo({ modalUserId = -1 }) {
  ...
  useEffect(() => {
    if (user) {
      if (modalUserId === -1) {
        ...
        setUserId(modalUserId)
        fetchProfileInfo(modalUserId)
      }
    }
  }, [user])
  ...
  const fetchProfileInfo = async (userId) => {
    try {
      ...
      if (profileResponse.data.userType) {
        const tutorRatingResponse = await axios.get(
          `${API_URL}/tutor/${userId}/rating`
        )
    ...
    >
      ...
      {tutorRatings.length > 0 && (
        <Box mt={2}>
          ...
          {showMoreClicked && (
            <Button onClick={handleShowLessReviews}>Show Less Reviews</Button>
          )}
        </Box>
      )}
      <Button
