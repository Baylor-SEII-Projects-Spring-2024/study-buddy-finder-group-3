...
function ProfileDisplay({ editable = true, uniqueId = -1 }) {
  ...
  const handleSaveClick = async () => {
    try {
      console.table("original profile", originalProfile)
      console.table("profile", profile)
      if (profile.username !== originalProfile.username) {
        console.log("user changed")
        const usernameCheckResponse = await axios.get(
          `${API_URL}/auth/checkUsername/${profile.username}`
        )
        console.log("usernameCheckResponse", usernameCheckResponse)
        if (!usernameCheckResponse.data) {
          console.log(profile.username + " already in database")
          toast.error(profile.username + " already belongs to another account")
          return
        }
      }

      if (profile.emailAddress !== originalProfile.emailAddress) {
        console.log("email changed")
        const emailCheckResponse = await axios.get(
          `${API_URL}/auth/checkEmail/${profile.emailAddress}`
        )
        if (!emailCheckResponse.data) {
          console.log(profile.emailAddress + " already in database")
          toast.error(
            profile.emailAddress + " already belongs to another account"
          )
          return
        }
      }
      ...
      const response = await axios.put(
        `${API_URL}/profile/updateProfile/${userId}`,
        updatedProfile,
        {
          headers: {
      ...
      if (response.status === 200) {
        setOriginalProfile(toBeSetToOriginal)
  ...
  const handleCourseChange = (event) => {
    ...
    if (checked) {
      ...
      setSelectedCourses((prevSelectedCourses) =>
        prevSelectedCourses.filter((course) => course !== name)
      )
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (event) => {
        ...
        img.onload = async () => {
          ...
          const compressedBlob = await fetch(compressedDataUrl).then((res) =>
            res.blob()
          )
  ...
  const handlePhotoChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/profile/updateProfilePhoto/${userId}`,
        formData,
        {
          headers: {
      ...
      >
        <div className={styles.coursesContainer}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedCourses.includes("Computer Science")}
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedCourses.includes("Biology")}
                onChange={handleCourseChange}
                name="Biology"
              />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedCourses.includes("Physics")}
                onChange={handleCourseChange}
                name="Physics"
              />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedCourses.includes("Mathematics")}
                onChange={handleCourseChange}
                name="Mathematics"
              />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedCourses.includes("Chemistry")}
                onChange={handleCourseChange}
                name="Chemistry"
              />
            }
      ...
      >
        <div className={styles.coursesContainer}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedMeetingType === "physical"}
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={selectedMeetingType === "virtual"}
                onChange={handleMeetingTypeChange}
                name="virtual"
              />
            }
